...
impl Database {
    ...
    pub async fn insert_blocks(&mut self, blocks: &[Block]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        Self::copy_data(&transaction, "blocks (block_hash, height, time, difficulty, merkle_root, nonce, size, version, bits, previous_block, active)", &block_lines).await?;
        Self::copy_data(&transaction, "transactions (txid, block_hash, size, version, locktime)", &tx_lines).await?;
        Self::copy_data(&transaction, "inputs (txid, input_index, previous_txid, previous_output_index, script_sig, sequence)", &input_lines).await?;
        Self::copy_data(&transaction, "outputs (txid, output_index, value, script_pub_key)", &output_lines).await?;
    ...
    async fn copy_data(transaction: &Transaction<'_>, table: &str, lines: &[String]) -> Result<(), Box<dyn std::error::Error>> {
        let mut sink: std::pin::Pin<Box<CopyInSink<bytes::Bytes>>> = Box::pin(transaction.copy_in(&format!("COPY {} FROM STDIN WITH DELIMITER ',' CSV", table)).await?);
        for line in lines {
            sink.as_mut().send(line.clone().into()).await?;
        }
